...
*Neutrino rows (ranks 1-3).* The $`R_1`$ sector spans 0.87, 7.3, 66.7 meV in ascending order (triv, std, gal): the lightest mass is unmeasured, the solar scale $`\sqrt{\Delta m^2_{21}} \approx 8.6`$ meV sits 0.85 of rank 2, the atmospheric scale $`\sqrt{\Delta m^2_{31}} \approx 50.6`$ meV sits 1.32 of rank 3. These are proxy comparisons, not assignments and not scorecard hits: all three absolute masses are experimentally unknown, and no generation is claimed from proximity alone. See §V for the splitting-level comparison and its mass-ordering test.
