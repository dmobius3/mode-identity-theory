...
|---|---|---|
| Solar: $`\sqrt{\Delta m^2_{21}}`$ | $`\approx 8.65`$ meV | $`\sim 4\,\mu_\Lambda`$ |
| Atmospheric: $`\sqrt{\Delta m^2_{31}}`$ | $`\approx 50.1`$ meV | $`\sim 22\,\mu_\Lambda`$ |

The ratios (≈4, ≈22) are observational comparison ratios, the measured splitting scales ([NuFIT 6.0](https://arxiv.org/abs/2410.05380), normal ordering) over $`\mu_\Lambda`$, not outputs of the mass formula; the formula's own $`R_1`$ entries sit at 0.39, 3.26, and 29.6 $`\mu_\Lambda`$ (§V).
...
| 2 | $`R_1`$ | 1 | std | $`7.33 \times 10^{-12}`$ | solar-scale proxy (not a hit) | $`8.65 \times 10^{-12}`$ | 0.85 |
| 3 | $`R_1`$ | 1 | gal | $`6.67 \times 10^{-11}`$ | atmospheric-scale proxy (not a hit) | $`5.01 \times 10^{-11}`$ | 1.33 |
...
*Neutrino rows (ranks 1-3).* The $`R_1`$ sector spans 0.87, 7.3, 66.7 meV in ascending order (triv, std, gal): the lightest mass is unmeasured, the solar scale $`\sqrt{\Delta m^2_{21}} \approx 8.65`$ meV sits 0.85 of rank 2, the atmospheric scale $`\sqrt{\Delta m^2_{31}} \approx 50.1`$ meV sits 1.33 of rank 3. These are proxy comparisons, not assignments and not scorecard hits: all three absolute masses are experimentally unknown, and no generation is claimed from proximity alone. See §V for the splitting-level comparison and its mass-ordering test.
...
The $`R_1`$ sector spans 0.87 meV (triv), 7.3 meV (std), 66.7 meV (gal) in ascending order, against the observed proxy scales: an unmeasured lightest mass, the solar scale 8.65 meV (ratio 0.85 to rank 2), and the atmospheric scale 50.1 meV (ratio 1.33 to rank 3). The three entries sit in ordered qualitative resemblance to the three observed scales. At the splitting level, taking the ladder as absolute masses would give $`\Delta m^2_{21} \approx 5.3 \times 10^{-5}`$ eV² and $`\Delta m^2_{31} \approx 4.45 \times 10^{-3}`$ eV² against the measured $`7.49 \times 10^{-5}`$ and $`2.51 \times 10^{-3}`$ eV²: ratios 0.71 and 1.77, an ordered resemblance, not a close reproduction of both splittings. All three absolute neutrino masses are experimentally unknown, the solar and atmospheric values are splitting proxies rather than masses, so these rows are proxy comparisons carrying no scorecard weight, and no generation assignment is claimed from proximity. As absolute masses the ladder is excluded: its splittings sit at the ratios above, and [NuFIT 6.0](https://arxiv.org/abs/2410.05380) fixes both to within about 15% and 6% at 3σ. The proxy comparison treats the solar and atmospheric scales as the second and third masses, which holds only in a normal ordering whose lightest mass lies well below the solar scale, as the ladder's does; an inverted ordering has no mass state at the solar scale. The reading is falsified if the NuFIT global fit or a successor favors the inverted ordering with $`\chi^2_\text{NO} - \chi^2_\text{IO} \geq 9`$. A normal ordering would satisfy the premise's ordering condition, not test the ladder's numbers.
...
| $`R_1`$ neutrino ladder | The sector sits in ordered qualitative resemblance to the observed proxy scales (§V: splitting-level ratios 0.71 and 1.77); proxy comparisons only, no assignments, no scorecard weight; falsified by an inverted mass ordering (§V). |
