...
*Dataset-convention note added 2026-09-01, post-freeze; no prediction cell, contender cell, or falsification threshold is edited.* **The $`2.90 \times 10^{-122}`$ benchmark above is the Planck 2018 +BAO combination this card carried at freeze**. The live framework now names TT,TE,EE+lowE+lensing as its Λ-anchored default, giving $`\Lambda\,\ell_P^2 = 2.845 \times 10^{-122}`$ ([Inputs and Calibration](../../framework/README.md#inputs-and-calibration)). The frozen figure is left as deposited; this clarification changes no prediction, threshold, or scoring rule, and Row I's prediction cell carries no numeric value in any case. The same applies to notation: the prose above now writes the computed $`3/R^2`$ as $`\Lambda_\text{ref}`$, matching the live framework, while three immutable cells keep the deposited form: Row I's Prediction Summary cell, the MIT row of the contender table in this section, and the Scoreboard. Those also carry "is topologically fixed", which the prose above now states more precisely; they are prediction cells under ground rule #1 and cannot be edited, which is why the correction lives in the prose rather than in them. The split is permanent by design, and $`\Lambda_\text{obs}`$ elsewhere in this section means the observed value $`R`$ was back-read from, which is the point of that sentence.**
...
*Re-registration of Row I, added 2026-09-21: after the 2026-06-05 freeze and before any Euclid DR1 data. The text of every cell on this card is unchanged.* **Row I is re-registered here, not repaired.** Its cells and threshold stay on the card as frozen and keep their 2026-06-05 date. That registration left one choice open, how matter is treated in the reconstruction, and this entry supplies it; it does not claim that the rule below was part of the June registration or is what its text meant. Because the rule changes what Row I tests, it is treated as an edit under ground rule 1: the 2026-06-05 registration of Row I is void, and Row I is registered anew on 2026-09-21. DR1 adjudicates Row I under this registration, and its result will be reported as the outcome of a test registered on that date, not of the June freeze.

**Why the frozen row does not determine a result.** A reconstruction of $`\Omega_\text{DE}(z)`$, the dark-energy density in the sense the w<sub>0</sub>w<sub>a</sub>CDM cell above uses it ("time-varying dark energy density; $`\Omega_\text{DE}(z)`$ not flat"), first subtracts a matter term from $`E^2(z) = A(1+z)^3 - \beta(1+z) + \Omega_\Lambda`$, with $`A = \Omega_m/(1-s_0^2)`$ and $`\beta = \Omega_m s_0^2/(1-s_0^2)`$. For $`s_0 > 0`$ no constant matter coefficient leaves an exactly flat residual, because a $`(1+z)^1`$ term cannot be absorbed into a $`(1+z)^3`$ one, so different treatments draw different histories from the same $`H(z)`$. Holding matter at the fiducial fraction, the split Row III names, leaves a residual that rises toward the past by $`\beta[(1+z)^3 - (1+z)]`$: relative to today, 1.3% at $`z = 0.9`$ and 5.1% at $`z = 1.8`$ at the SN+BAO posterior median $`s_0 = 0.076`$, and 8.5% and 33.0% at the 95% bound $`s_0 = 0.19`$. Subtracting the dressed matter term $`A(1+z)^3`$ leaves $`-\beta(1+z)`$ ([dark energy](dark-energy.md) §III).

**The registered rule, chosen against the framework.** Row I is scored on two binned reconstructions in the DR1 BAO bins, one holding matter at the fiducial fraction and one fitting it with the bins, and it fails if either varies at $`\geq 2\sigma`$ across the bins. Other reconstructions are reported alongside and do not score. The union is conservative by intent: with two looks, the chance that noise alone fails the row is higher than for a single 2σ test, up to twice as high, and that price falls on the framework. The row is not declared unscorable instead. The card gives it no deferral provision, and the DESI fit in the w<sub>0</sub>w<sub>a</sub>CDM cell above implies a dark-energy density that evolves, so setting the row aside would remove the framework's exposure to the very outcome that fit implies.

**The adverse direction, stated plainly.** "Flat" now means flat to within 2σ at Euclid's precision, and the framework's own background predicts a rise in the fiducial arm. Row I is therefore a joint test: it can fail on an evolving vacuum term, and it can also fail on the framework's own phase if $`s_0`$ sits near the upper end of its allowed range. It reads the same $`H^2(z; s_0)`$ background as Rows II, III and V, so the accounting of the 2026-06-26 note extends to it: passing Rows I, II, III and V together would confirm one relation through four channels, not four independent successes.

**The rule this sets.** A row on this card may be re-registered only before its data, only in the direction that makes it harder for the framework, and only under its own date.
...
*Clarification added 2026-09-02, post-freeze; no prediction cell, contender cell, Scoreboard cell, or falsification threshold is edited.* **The prose above stated this row's core claim unscoped, then scoped it in the next sentence.** The companion analysis scopes $`w_\text{eff}(z) > -1`$ to the fiducial matter split and notes that a different valid split reads $`w_\text{eff} < -1`$ at high $`z`$, so there is no decomposition-independent statement about $`w(z)`$ to make; the decomposition-independent claim is that the underlying $`H(z)`$ introduces no independent phantom fluid, and the prose now says that. The same analysis states that its template exercise establishes the mechanism rather than showing DESI's signal to be primarily template bias, so the prose now treats template projection as a candidate explanation, matching the callout below rather than overrunning it. **This leaves the same asymmetry the 2026-09-01 note records for $`\Lambda`$.** The Scoreboard's "$`w > -1`$ at all $`z`$; any apparent crossing is template artifact" is now the card's only unscoped statement of either claim, as Row I's cell is its only untyped $`\Lambda`$, and it stays for the same reason: a pre-registration is worth what it committed to, not what it would commit to now.
...
> 🎯 *Outcome to be recorded after DR1-Foundation, November 2026. The wide-area photometric source catalogue and the spectra ship at that stage, which puts this row roughly half a year ahead of the other four.* *(Superseded on 2026-09-23 by the re-registration below: the outcome is recorded when the first qualifying measurement appears, at the Foundation stage or later.)*

*Re-registration of Row IV, added 2026-09-23: after the 2026-06-05 freeze and before any Euclid DR1 data. The text of every cell on this card is unchanged.* **Row IV is re-registered here, not repaired.** Its cells and threshold stay on the card as frozen and keep their 2026-06-05 date. That registration left open what its comparison is, which statistic and which measurement score, how its 2σ is computed, and when it scores. This entry fixes them; it does not claim they were part of the June registration or are what its text meant. Because they change what Row IV tests, the entry is treated as an edit under ground rule 1: the 2026-06-05 registration of Row IV is void, and Row IV is registered anew on 2026-09-23. Its scope does not change: the row reads Euclid-based wide-area measurements, as its cells say, from DR1 or a later release; quick-look products stay outside the card, as its scope field says.
