...
*Clarification added 2026-09-25, before any Euclid DR1 data.* **The MIT cell's reachability is a conditional translation.** The $`a_0(z)`$ evolution is the registered MIT prediction; translating it into the cell's reachability under standard $`\varepsilon_\text{SF} \lesssim 1`$ uses the standard deep-MOND relation, conditionally ([early galaxies](early-galaxies.md#collapse-dynamics)). This note changes no cell, threshold or scoring rule.
