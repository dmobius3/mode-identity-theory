...
| R7 spatial projection | Derive the spatial part of $`g_\text{static} \to g_\text{eff}`$ from the closed substrate: the curvature placement of §VI, together with the fluctuation map that placement induces on the shells; the Λ-dressing of the clock stays with the clock and action side | (b) | C7 and C8, and the curvature-placement half of §VI's two-part metric-definition problem; C3 bars reading the placement off the fitted dictionary, as it bars R2 | OPEN; two calculations run: the directional transfers classified shell by shell ([The Shell-to-Direction Transfers](directional-transfers.md)), and the radial transfers ([The Shell-to-Radius Transfers](radial-transfers.md)) |
