...
*Added 2026-09-25: the two statements above that MIT keeps gravity inverse-square everywhere and predicts it exact at all scales are retired. The framework's claim is narrower: it does not modify the local gravitational field equations ([engine](../../../README.md#gravity-is-what-crosses)), and its $`a_0`$ sector predicts an evolving scale in resolved galaxy dynamics, whose entry into the effective metric or source term is not derived; [early galaxies](../../../../cosmos/files/early-galaxies.md#collapse-dynamics) translates it through the deep-MOND relation, conditionally. With the broader claim retired, the Gallardo et al. result no longer discriminates in the framework's favor; it bears instead on the response the framework has not derived. Their $`n = 2.1 \pm 0.3`$, for the gravitational acceleration $`g \propto r^{-n}`$ between pairs of halos at 30 to 230 Mpc, constrains any extension of the deep-MOND $`1/r`$ point-mass response to those separations. That response gives $`n = 1`$, $`3.7\sigma`$ from their value, computed here as $`(2.1 - 1)/0.3`$. A source bound to halos acts between them as ordinary mass does, $`n = 2`$, which the data favor.*
...
The geometric mechanism behind $`L_f`$ as a coherence scale is explored in the [cone point coherence notes](cone-point-coherence.md), which ask whether the $`W`$-independence of the zonal first-positive eigenvalue nests to galactic scale. On the cosmic band that independence holds because the zonal equation contains no $`W`$ ([first-eigenvalue paper](../../bedrock/files/first-eigenvalue.md) §4.1), and the eigenvalue is first positive only for the narrow band, $`W \le \pi R/2`$ (Theorem 1.2). The SPARC test is empirical; the cone point analysis is structural. Both probe the same object ($`L_f`$) from different directions.
