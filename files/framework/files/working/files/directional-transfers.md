...
**Inputs:** `molien-p1-bridge.md` §II, §V-VI, `molien-shells.md`, `stress-tensor-bridge.md` C7 and R7, `../../bedrock/files/surviving-ray.md` §2.2, §2.3, §3.4, §4.1, §5.7, §5.9, `scripts/directional-transfer/transfer_classification.py`
...
Conversely, every such diagonal $`E_N(\hat z)`$ defines a covariant family, since the rotations carrying $`\hat z`$ to $`\hat n`$ differ only by a rotation about $`\hat z`$, which commutes with it. **(Pointer added 2026-09-15.)** The classification assumes this covariance under every rotation of the flat target; [The Effective-Metric Floors](effective-metric-floors.md) records its status, and the Stress-Tensor Bridge carries it as C10. [The Spatial Carriers](spatial-carriers.md) then asks which covariance transfers admit deterministic field-level shell lifts. In the full-rotation branch no nonzero finite-shell lift survives; under only $`A_5`$ equivariance, outside that full-rotation family, specific atomic carriers can exist.
