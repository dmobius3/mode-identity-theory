...
**Type:** Map
**State:** Active
**Status (2026-09-21):** Framework-wide adversarial classification of every quantitative claim; the reconciliation and eight-cycle web are current. Cycles 2 (α as both input and output) and 7 (the R routes 3.2× apart in R on the canonical electron-muon pair, 1.7× on the best assigned pair) remain the live tensions; the other six are consistency loops, definitions, or presentation double-counts, Cycle 6 among them (Ω_m is the anchor echoed back, not a validation of the clock exponent). The mass-formula entry rule is carried as an open since 2026-09-04. Euclid Row I was re-registered on 2026-09-21, before any DR1 data, and joins the phase-clock channels as a correlated test.
...
| 3 | Dilution (√Ω)^−n / factorization | any dilution law; factored vs additive/mixed forms | **dimensionally constrained + unresolved**: once the single-ratio monomial ansatz and the Planck reference are fixed, units fix the integer power for dimensional observables; the factorization from C(Θ), the choice of √Ω as the hierarchy variable, and the geometric-seat reading of n ride the open commutant theorem (two rows up) | Λ, H₀/a₀, dark-null |
