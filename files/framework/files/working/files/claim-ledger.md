...
| Boundary-mode uniformity: every contributing spectral block carries the first-positive profile times a fixed spectral weight, K_l(Θ) = C(Θ) A_l; for single-position observables the factored scaling law is equivalent to it by a derived lemma, and the mass sector's geometric-mean seat is its own question (restated 2026-09-23 from the commutant theorem A_obs = A_Θ ⊗ A_spec, which cannot forbid a cross term) | load-bearing | keystone gap; whole factored measurement form is conditional |
...
| 2 | Phase operator C(Θ) | any rule on the anti-periodic BC | **conditional theorem**: unique given the admissible class, the first-positive rule, and unit-mean normalization; the BC alone does not give these (they are named selection principles, per scaling-law-uniqueness) | everything |
