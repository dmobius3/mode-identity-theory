...
**The second requirement's level, recorded 2026-09-15.** Requirement 2 asks for a map from fields to fields, and requirement 3 asks for linearity and positivity so that the object lands in the classified family, which is a family of maps on covariances. Read together, requirement 2 fixes the object's type and requirement 3 constrains the transfer that type induces, and that is the reading kept here: the object is field to field. Both readings were available before [The Spatial Carriers](spatial-carriers.md) was computed, and this one is kept now that the computation has a consequence, rather than re-read in the direction that would preserve P1. The consequence is owned: P1, a covariance-level prescription whose numbers stand, is not the induced transfer of any deterministic field-level map at finite shell dimension under separate shell homogeneity. Across the shells tested there the field-level branch produces no replacement for [the ratio table](molien-ratio-table.md): $`N = 20`$ and $`N = 30`$ admit $`A_5`$ carriers whose contributions to $`C_\ell`$ equal that table's, $`N = 12`$ admits none once locality is imposed, and those shellwise carriers do not assemble into an admissible field-level lift.
