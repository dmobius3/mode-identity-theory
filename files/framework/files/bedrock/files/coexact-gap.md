...
Let $`X = S^3/\Gamma`$ be a homogeneous spherical space form, the quotient of the round three-sphere of radius $`R`$ by a finite subgroup $`\Gamma \subset \mathrm{SU}(2)`$ acting by left translation, and let $`E_\tau`$ be the flat bundle determined by a finite-dimensional representation $`\tau`$ of $`\Gamma`$. What is the lowest nonzero eigenvalue of the Hodge Laplacian on coexact $`E_\tau`$-valued $`1`$-forms, and how does it depend on $`\tau`$?

The gap is always $`q_\tau^2/R^2`$ for an integer $`q_\tau`$, the first level of the round sphere's coexact spectrum at which a constituent of $`\tau`$ appears, and it is likewise the square of the smallest absolute eigenvalue of the natural curl-type operator on the same forms. The spectral question therefore reduces to a first-occurrence problem in representation theory, and the McKay correspondence between finite subgroups of $`\mathrm{SU}(2)`$ and the affine ADE diagrams answers that: an irreducible representation first occurs in the restricted symmetric-power tower at its graph distance from the trivial node. The two levels agree whenever the nearest constituent of $`\tau`$ lies at distance at least two, and the gap is then the squared distance divided by $`R^2`$ (Theorem 4.2).

The principal specialization is the adjoint bundle of an irreducible flat $`\mathrm{SU}(2)`$ connection, the coefficient system of the gauge-theoretic deformation problem. Its constituents always lie at McKay distance at least two, so the distance rule applies to it without qualification, and the answer is uniform across the ADE classification of finite subgroups of $`\mathrm{SU}(2)`$ with exactly one exception.
...
The adjoint case carries a gauge-theoretic reading, and it is the reason this coefficient system is the natural one to isolate. The eigenvalue of Theorem 1.1 is the bottom of the gauge-fixed Laplacian on the Coulomb slice at the connection: the slice is cut out by the coclosedness condition $`d_\nabla^* a = 0`$, and coclosed $`1`$-forms coincide with coexact ones on these spaces (Lemma 2.1). The Poincaré homology sphere $`\Sigma(2,3,5) = S^3/2I`$, which carries the exceptional pair, is the unique integral homology sphere among spherical space forms with nontrivial fundamental group, and was singled out on that account by Fintushel and Stern [FintushelStern]; adjoint-coefficient computations for it appear in [Nasatyr1992, KirkKlassen], with the binary polyhedral character and flat-connection data recorded by Helle [Helle]. Those are spectral-flow computations, comparing connections along paths; the quantity computed here is intrinsic to each connection separately.

Three classical spectral ingredients, recorded next, combine with the McKay correspondence [McKay] into the first-occurrence rule of Theorem 4.2, and Theorem 1.1 follows from it. Only the graph metric of the correspondence enters here.
