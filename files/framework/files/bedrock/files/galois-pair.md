...
The augmentation reaches the boundary as well. Combining Proposition 5.3 with Theorem 1.1 in the fundamental sector,

```math
\rho_{Q'}-\rho_Q\;=\;4\bigl(k(\mathcal{R}_Q)-k(\mathcal{R}_{Q'})\bigr)\;=\;\frac{4\,\varepsilon(H)}{\lvert 2I\rvert}\;=\;-\frac{12}{5}.
```

So $`\varepsilon(H)`$ controls three quantities at once: Helle's Chern-Simons difference modulo $`1`$, the tautological charge difference exactly, and the fundamental rho difference exactly. The exchange factor $`\Delta\rho=-4\,\Delta k`$ recorded in the proof of Theorem 1.2 is thereby exhibited in the fundamental sector as well as the adjoint one.
