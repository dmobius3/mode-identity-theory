...
>**📗 [First Eigenvalue](files/bedrock/files/first-eigenvalue.md):** the twisted Möbius Laplacian in full, fixing the first positive surface level, $`\Lambda_\text{top} = 2/R_\Lambda^2`$.
...
The full $`2I`$ sampling resolution carries 120 steps, and its central-sign quotient $`2I \to I`$ has 60 elements ($`\lvert I\rvert = 60`$), the count of the 60R grid. Integer-spin representations factor through $`I`$; half-integer ones, the fermions', do not factor through $`I`$, and spinorial roles are assigned the full 120-step resolution. That two resolutions exist is the representation theory; which observable reads on which is a separate step, taken by two rules. Ladder roles go by spin parity, as above; the cosmological observables go by a motivated assignment with no representation-theoretic ground, photon-mediated quantities to 60R and dynamical ones, $`a_0`$ among them, to the full 120. The representation theory under the first rule is derived; neither assignment is. The grids differ in resolution, not in how they are read. Both use the same readout, the intensity or squared amplitude, and the intensity is even, so it cannot see the central sign in any sector: on a half-integer-spin fiber the central element acts as $`-1`$. Nor is the 120 grid a set of 120 distinguishable readings of deck copies, since the scalar intensity is the same on all 120 translates; it is the finer phase resolution, whose odd numerators over 120 are the positions 60R lacks, $`a_0`$ at 13/120 among them. The quotient and the 60R grid share a count, but the present structure supplies no canonical pointwise map between them. Within $`2I`$ and $`I`$ themselves there is none: both are perfect, so neither has a nontrivial homomorphism onto a cyclic ruler, and neither contains an element of order 120 or 60. Matching their elements to phase positions one by one would take additional structure the framework does not supply. The Möbius sign flip and the central $`-I`$ of $`2I`$ are two distinct $`Z_2`$'s; the intensity is blind to both for the same algebraic reason, and the natural sampler built from the Möbius geometry does not carry one onto the other.
...
| 60R resolution | 60 | 2/120 | $`H_0`$, $`\Lambda`$, $`\alpha`$ (photon-mediated) |
...
Restrict the propagating mode $`\rho \otimes \sigma`$ to the three-fold face stabilizer $`Z_3`$, and it decomposes into trivial characters and conjugate nontrivial character pairs. The trivial channels are the color singlets and the nontrivial pairs the color triplet and anti-triplet channels. The decomposition determines which color channels are available; the full $`(\rho, \sigma)`$ address determines which one a particle occupies. The face structure is vacuum-independent, so color is generation-independent.
...
The vacant fourth rung has a SUSY reading. The one missing pairing would turn a fermion into a boson, exactly the move a superpartner symmetry asks for, and the standard supersymmetric gaugino-scalar gauge sector is its natural occupant. Every real gauge rung acts within a statistics class: EM, strong, and weak change phase, charge, or representation but leave the fermion or boson character of what they act on intact. A superpartner rung would instead pair a fermionic carrier on the 120-grid with a bosonic target on the 60R-grid, across the split the central element $`-I`$ carries between half-integer and integer spin, and no gauge rung crosses it. The obstruction is the ladder's own rule alone: not a spin-statistics prohibition (ordinary quantum field theory permits fermionic mediators), and not a property of the readout, since every slot is read through the same intensity. So within the grid-action reading the missing superpartner force is not an unreached energy scale but the empty fourth chair in a four-rung ladder: a conditional structural prediction of the ladder, not a spin-statistics theorem and not a claim that supersymmetry is impossible at every scale.
...
The mass spectrum assembles in three moves, each set by the same topology: the curvature gap on the coexact gauge modes, the three flat vacua, and the four-factor formula that ranks the fermions.
