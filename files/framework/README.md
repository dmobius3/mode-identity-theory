...
The full $`2I`$ sampling resolution carries 120 steps, and its central-sign quotient $`2I \to I`$ has 60 elements ($`\lvert I\rvert = 60`$), the count of the 60R grid. Integer-spin representations factor through $`I`$; half-integer ones, the fermions', do not factor through $`I`$, and spinorial roles are assigned the full 120-step resolution. That two resolutions exist is the representation theory; which observable reads on which is a separate step, and reading photon-mediated quantities onto 60R is a motivated grid assignment, not a derivation. The grids differ in resolution, not in how they are read. Both use the same readout, the intensity or squared amplitude, and the intensity is even, so it cannot see the central sign in any sector: on a half-integer-spin fiber the central element acts as $`-1`$. Nor is the 120 grid a set of 120 distinguishable readings of deck copies, since the scalar intensity is the same on all 120 translates; it is the finer phase resolution, whose odd numerators over 120 are the positions 60R lacks, $`a_0`$ at 13/120 among them. The Möbius sign flip and the central $`-I`$ of $`2I`$ are two distinct $`Z_2`$'s; the intensity is blind to both for the same algebraic reason, and the natural sampler built from the Möbius geometry does not carry one onto the other.
